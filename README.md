...
The states and actions are identified by what is called a *token*. They are respectively *StateTokens* and *ActionTokens* classes. (It differs for JavaScript runtime)
...
Finally, instance a *StateManager* class, register the states using its *RegisterState* method, and call its *SetInitialState* method to provide an entry point to the state machine.
...
In the following document, a shopping basket is used as sample for the purpose of explanation.
...
A state have to register action handlers, they tell the state machine of which actions the current state is aware of. This is best done in the *OnInitialize()* virtual method.
...
Instance a *StageManager* object, register states and set the initial state, as follow:

	var sm = new StateManager();

	sm.RegisterState(new ProductListBasketState());
	sm.RegisterState(new PaymentBasketState());
	sm.RegisterState(new ConfirmationBasketState());

	sm.SetInitialState(BasketStateTokens.ProductList);
...
bitcraft offers a tool to generate base state machine code (sub class of state manager, state tokens, action tokens, specific states and handler registration) from a .graphml diagram file.

Note that the tool only generates C# code. Generating JavaScript makes few sense, and generating C++ is not planned to be developed, so if you need this feature, please implement it by yourself, based on what the tool already provides.

We use yEd from yWorks: [Download page](http://www.yworks.com/en/products_yed_download.html)

For the moment we didn't test any other tools, neither other .graphml files generated with other tools.
...
- Bitcraft.ToolKit.CodeGeneration
	...
	- We tried to abstract code generation as much as possible, but found out on the way that some languages have few things in common, and thus it doesn't make that much sense to try to abstract that.
	- The [Roslyn](https://roslyn.codeplex.com) library could probably be used, but we estimated it a bit overkill for this usage, though could have been very interesting to investigate and learn.
- Bitcraft.StateMachineTool.Core
	- Contains the contracts the tool needs to parse and load oriented graphs.
...
So in the handler, you have to cast *self* into the type of your state, not a big deal. You can still access the non public members if the function is declared into the class.
