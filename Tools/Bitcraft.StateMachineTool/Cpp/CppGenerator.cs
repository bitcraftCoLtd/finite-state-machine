...
public sealed class CppGenerator : IGenerator
{
    private static string GetMandatoryKeyValue(IReadOnlyDictionary<string, string?> customOptions, string key, string helpMessage)
    {
        ...
        if (string.IsNullOrWhiteSpace(value))
            throw new Exception($"Custom option '{key}' value is not set. {helpMessage}");
    ...
    private static void GenerateStateMachineCode(ILanguageAbstraction sourceLanguageAbstraction, ILanguageAbstraction headerLanguageAbstraction, GeneratorOptions options)
    {
    ...
    private static void GenerateStatesCode(ILanguageAbstraction sourceLanguageAbstraction, ILanguageAbstraction headerLanguageAbstraction, GeneratorOptions options)
    {
        string projectRelativePathPrefix = GetProjectRelativePathPrefix(options.CustomOptions);
        string generatedCodeRelativePathPrefix = GetGeneratedCodeRelativePathPrefix(options.CustomOptions);
        string? stateMachineRelativePathPrefix = GetStateMachineRelativePathPrefix(options.CustomOptions);
